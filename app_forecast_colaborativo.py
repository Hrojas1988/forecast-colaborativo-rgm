...
# -------------------------------------------------------------------
# 5. Precio editable por SKU
# -------------------------------------------------------------------
st.subheader("6. Precio proyectado por SKU (editable)")
precio_default = hist.groupby("SKU")["Precio"].last().reset_index()
precios = st.data_editor(precio_default, num_rows="fixed", use_container_width=True, key="precios")
...
final = final.merge(precios, on="SKU", how="left")
...
cols_mostrar = ["Cliente", "SKU", "Mes", "BAU_Unidades", "%_Colaborativo", "Unidades_Final", "Precio", "Venta_Final"]
...
# 7. Exportar a Excel
# -------------------------------------------------------------------
def exportar_excel(hist, coef_df, escenario, iniciativas, precios, final, resumen) -> bytes:
    wb = Workbook()
...
excel_bytes = exportar_excel(hist, coef_df, escenario, iniciativas, precios, final, resumen)
st.download_button(
